...
void FerNNClassifier::prepare(const vector<Size>& scales){
  ...
  for (int i=0;i<totalFeatures;i++){
      ...
      for (int s=0;s<scales.size();s++){
		  ...
		  //给第s种尺度的第i个特征随机存入两个坐标点，用于以后的图像比较
...
void FerNNClassifier::getFeatures(const cv::Mat& image,const int& scale_idx, vector<int>& fern){
  ...
  for (int t=0;t<nstructs;t++){
      leaf=0;
	  //每棵树有13个叶子，也就是13个节点，leaf是一个13位的数，表示的是每一棵树的特征
      for (int f=0; f<structSize; f++){
		  //比较第scale_idx种尺度变换的第t*nstructs+f个特征的灰度值
		  //在image图像中比较指定的两个点的灰度大小，作为一个特征
		  //也就是如果（y1,x1）的灰度大于（y2,x2）的灰度，则返回1，否则返回0
          leaf = (leaf << 1) + features[scale_idx][t*nstructs+f](image);
      }
	  //把10棵树的特征都保存在fern容器里面
      fern[t]=leaf;
  }
}
...
//这个训练是用来更新正负样本的后验概率的，是为集合分类器服务的
...
//如果标签是正样本（正样本的标签是1，是当前这次循环
//重新赋予的，也就是一个新的标签）                          //       double *x = X+nTREES*I; //tree index
          if(ferns[i].second==1){                           //       if (Y[I] == 1) {
//如果上一次循环的后验概率表明当前的这个patch是负样本，后验概率小于正样本的阈值
//那这就说明我要把这个看似是负样本的patch变成正样本了，要以当前循环的结果为准，
//上次循环的那个不准了，这样我就要更新这个patch的后验概率了
              if(measure_forest(ferns[i].first)<=thrP)      //         if (measure_forest(x) <= thrP)
                update(ferns[i].first,1,1);                 //             update(x,1,1);
//如果标签是负样本（负样本的标签是0，是当前这次循环
//重新赋予的，也就是一个新的标签）
          }else{                                            //        }else{
//如果上一次循环的后验概率表明当前的这个patch是正样本，后验概率大于负样本的阈值
//那这就说明我要把这个看似是正样本的patch变成负样本了，要以当前循环的结果为准，
//上次循环的那个不准了，这样我就要更新这个patch的后验概率了
...
//训练最近邻分类器，传入的nn_examples的第一个图片是pEx，后面的都是重合度很低的patch
...
void FerNNClassifier::NNConf(const Mat& example, vector<int>& isin,float& rsconf,float& csconf){
  ...
  bool anyP=false;
  int maxPidx,validatedPart = (int)ceil(pEx.size()*valid);//valid = 0.5 在参数文件里
  float nccN, maxN=0;
  bool anyN=false;
  for (int i=0;i<pEx.size();i++){
      matchTemplate(pEx[i],example,ncc,CV_TM_CCORR_NORMED);      // measure NCC to positive examples
      nccP=(((float*)ncc.data)[0]+1)*0.5f;//计算正样本匹配相似度
      if (nccP>ncc_thesame)
        anyP=true;
      if(nccP > maxP){//记录匹配最大相似度，并记录索引
          maxP=nccP;
          maxPidx = i;
          if(i<validatedPart)
            csmaxP=maxP;
  ...
  for (int i=0;i<nEx.size();i++){//计算负样本匹配相似度
